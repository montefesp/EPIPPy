...
def match_points_to_regions_eez(all_points: List[Tuple[float, float]], shapes_dataset: pd.Series, eez_ds: pd.Series,
    ...
    assert len(all_points) != 0, "List of points is empty."
    assert len(shapes_dataset) != 0, "Series of shapes is empty."

    points_region_ds = pd.Series(index=pd.MultiIndex.from_tuples(all_points)).sort_index()
    all_points = MultiPoint(all_points)

    # Loop through all shapes and assign points intersect with them
    for i, s in eez_ds.items():

        points = all_points.intersection(s)
        regions_of_eez = shapes_dataset.index.str.contains(i)
        shapes_ds = shapes_dataset.loc[regions_of_eez]
